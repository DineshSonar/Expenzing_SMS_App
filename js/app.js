...
alert("in saveSMS ")
	if (mydb) {
		...
		if (smsMsg != "") {
	            mydb.transaction(function (t) {
	              t.executeSql("INSERT INTO BusinessExpDetailsForSMS (accHeadId,expNameId,expDate,expFromLoc, expToLoc, expNarration, expUnit, expAmt, currencyId, isEntitlementExceeded) VALUES (?,?,?,?,?,?,?,?,?,?)", 
				[accHeadId,expNameId,ocrExpDate,ocrFromLoc,ocrToLoc,ocrnarration,ocrUnit,ocrAmount,currencyId,isEntitlementExceeded]);
				});
             checkforData();
	        } else {
	        }
